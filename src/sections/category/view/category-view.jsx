...
import { useGetAllCategoriesQuery, useDeleteCategoryMutation, useDeleteCategoriesMutation } from 'src/app/api/category/categoryApiSlice';
...
export default function CategoryView() {
  ...
  useEffect(() => {
    if (categoryData) {
      const { totalPages } = categoryData.data;
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = categoryData?.data?.content?.map((n) => n.id);
  ...
  const handleDeleteSelectedItems = async () => {
    try {
      const { data } = await deleteCategories(selected).unwrap();
      showSuccessMessage(data);
    } catch (error) {
      handleError(error);
    }
          ...
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              <CustomTableHead
                order={order}
                orderBy={orderBy}
                rowCount={categoryData?.data?.content?.length}
                numSelected={selected.length}
